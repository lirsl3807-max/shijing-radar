    ...
    for kw in RELEVANCE_KEYWORDS:
        if kw in t:
            ...
            if kw == "国风" and "中国风" in t:
                continue
            # 特殊情况："诗谱"可能匹配"新诗谱"（现代诗书，非郑玄《诗谱》）
            if kw == "诗谱" and "新诗谱" in t:
                continue
            return True
    return False
...
def search_all():
    ...
    # 策略2: 按期刊名搜（覆盖所有CSSCI正刊 + 扩展版 + 集刊）
    print("\n策略2: 全面期刊检索（分批执行，防止限流）")
    key_journals = [
        # ═══ 高产出期刊 ═══
        "文史", "文献", "文学遗产", "中华文史论丛",
        "出土文献", "简帛", "中国经学", "孔子研究",
        ...
        "文津学志", "古籍研究", "中国诗学研究", "古代文学理论研究",

        # ═══ CSSCI 高校学报 ═══
        "北京大学学报", "清华大学学报", "复旦学报",
        "四川大学学报", "武汉大学学报", "华东师范大学学报",
        "上海交通大学学报", "南京大学学报", "浙江大学学报",
        "中国人民大学学报", "北京师范大学学报",
        "中山大学学报", "烟台大学学报",
        ...
        "甲骨文与殷商史", "经学研究", "历史文献研究",
        "语言学论丛", "诸子学刊", "诗书画",
        "中国美学研究", "中国语言文学研究", "文学理论前沿",
        "孔学堂", "国际儒学(中英文)",
        "古籍保护研究", "汉语史研究集刊",
        "历史地理", "中国历史地理论丛",
        "唐宋历史评论", "宋史研究论丛",
    ]
    for idx, jname in enumerate(key_journals):
        # 每批20个后休息5秒，防止知网限流
        if idx > 0 and idx % 20 == 0:
            print(f"  --- 已搜{idx}/{len(key_journals)}个，休息5秒防限流 ---")
            time.sleep(5)

        print(f"  [{idx+1}/{len(key_journals)}] {jname}...", end=" ", flush=True)
        data = run_cnki_search(jname, field="source")

        # 如果返回0条，可能被限流了，重试一次
        if data.get("total_hits", 0) == 0 and data.get("results", []) == []:
            time.sleep(3)
            data = run_cnki_search(jname, field="source")
